const products = [
  ...
  function calAvg(reviews, productId) {
  ...
  function displayProducts(filtered) {
    ...
    filtered.length != 0
      ? filtered.forEach((product) => {
          const avgRating = calAvg(reviews, product.id);
  
          const proCard = `
      <div class="proCard" data-product-id="${product.id}">
      <div id="sliderImage" >
      <img src="${product.images[0] || "https://via.placeholder.com/150"}" alt="${product.title}" />
        ...
        <span class="sprice">${product.strikePrice.toFixed(2)}</span>
        </p>
        <p class="quantity">Available: <span>${product.quantity}</span></p>
        <p class="category">Category: <span>${product.category}</span></p>
        <p class="rating">Rating: <span> ${avgRating || "No ratings"} 🌟</span> </p>
          <button id="addToCartBtn" class="add">Add to Cart</button>
          </div>
          </div>
          `;
          productDisplay.innerHTML += proCard;
        })
      : (productDisplay.innerHTML = `<div class="nofound">No results Found!</div>`);
  }
